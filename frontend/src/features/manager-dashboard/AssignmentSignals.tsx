...
function SignalCard({ label, value, isAmber = false, sublabel }: SignalCardProps) {
  return (
    ...
    >
      <p className={`text-xs font-medium uppercase tracking-wide ${isAmber ? 'text-amber-700' : 'text-gray-500'}`}>
        {label}
      </p>
      <p className={`text-2xl font-bold ${isAmber ? 'text-amber-800' : 'text-gray-900'}`}>
        {value}
      </p>
      {sublabel && (
        <p className={`text-xs ${isAmber ? 'text-amber-600' : 'text-gray-400'}`}>
          {sublabel}
        </p>
      )}
      {isAmber && (
...
export function AssignmentSignals({ signals }: AssignmentSignalsProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <h2 className="text-lg font-semibold text-gray-900">Assignment Signals</h2>
