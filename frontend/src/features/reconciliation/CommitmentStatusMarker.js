...
export function CommitmentStatusMarker({ value, onChange, disabled = false, }) {
    return (_jsx("div", { role: "radiogroup", "aria-label": "Reconciliation status", className: "flex flex-wrap gap-1", children: STATUS_OPTIONS.map((option) => {
            const isSelected = value === option.value;
            return (_jsx("button", { type: "button", role: "radio", "aria-checked": isSelected, disabled: disabled, onClick: () => onChange(option.value), className: [
                    ...
                    'focus:ring-blue-500',
                ].join(' '), children: option.label }, option.value));
        }) }));
}
