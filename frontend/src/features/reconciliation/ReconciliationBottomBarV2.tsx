...
interface ReconciliationBottomBarV2Props {
...
export function ReconciliationBottomBarV2({
  reconciledCount,
  totalCommitments,
  allReconciled,
  onComplete,
  loading = false,
}: ReconciliationBottomBarV2Props) {
  const pct =
    totalCommitments > 0
  ...
  return (
    <div
      className="fixed bottom-0 left-0 right-0 z-50 border-t border-outline-variant bg-surface-lowest/90 backdrop-blur-[20px]"
    >
      <div className="max-w-[960px] mx-auto px-8 py-3 flex items-center justify-between">
        ...
        <Button
          ...
          onClick={onComplete}
        >
          {loading ? 'Submitting\u2026' : allReconciled ? 'Close My Week' : 'Reconcile all to close'}
        </Button>
      </div>
    </div>
  );
