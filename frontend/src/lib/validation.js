...
}).refine((data) => !data.outcomeId || data.definingObjectiveId, { message: 'Defining Objective is required when Outcome is set', path: ['definingObjectiveId'] }).refine((data) => !data.definingObjectiveId || data.rallyCryId, { message: 'Rally Cry is required when Defining Objective is set', path: ['rallyCryId'] });
...
}).refine((data) => data.status === 'COMPLETED' || (data.completionNotes && data.completionNotes.trim().length > 0), { message: 'Notes are required when status is not Completed', path: ['completionNotes'] });
